...
def _pooling(net, stride, operation):
    ...
    if pooling_type == 'avg':
        net = slim.avg_pool2d(net, pooling_shape,
                              stride=stride, padding=padding)
    elif pooling_type == 'max':
        net = slim.max_pool2d(net, pooling_shape,
                              stride=stride, padding=padding)
    else:
...
class NasNetABaseCell(object):
    ...
    def _apply_conv_operation(self, net, operation,
        ...
        if 'separable' in operation:
            ...
            if stride > 1 or (input_filters != filter_size):
                net = tf.nn.relu(net)
                net = slim.conv2d(net, filter_size, 1,
                                  stride=stride, scope='1x1')
    ...
    # No public API. For internal use only.
    @tf.contrib.framework.add_arg_scope
    def _apply_drop_path(self, net, current_step=None,
        ...
        if drop_path_keep_prob < 1.0:
            ...
            if use_summaries:
                with tf.device('/cpu:0'):
                    tf.summary.scalar('drop_path_keep_prob',
                                      drop_path_keep_prob)
