...
class DCGANTest(tf.test.TestCase):
    ...
    def test_generator_graph(self):
        ...
        for i, batch_size in zip(xrange(3, 7), xrange(3, 8)):
            ...
            expected_names = ['deconv%i' %
                              j for j in xrange(1, i)] + ['logits']
            ...
            for j in range(1, i):
                ...
                self.assertEqual(
                    32 * 2**(i-j-1), layer.get_shape().as_list()[-1])
    ...
    def test_discriminator_graph(self):
        ...
        for i, batch_size in zip(xrange(1, 6), xrange(3, 8)):
            ...
            expected_names = ['conv%i' %
                              j for j in xrange(1, i+1)] + ['logits']
            self.assertSetEqual(set(expected_names), set(end_points.keys()))
            ...
            for j in range(1, i+1):
                ...
                self.assertEqual(
                    32 * 2**(j-1), layer.get_shape().as_list()[-1])
