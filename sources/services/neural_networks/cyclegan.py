...
def cyclegan_upsample(net, num_outputs, stride, method='conv2d_transpose'):
    ...
    with tf.variable_scope('upconv'):
        ...
        if method == 'nn_upsample_conv':
            ...
            net = layers.conv2d(net, num_outputs, kernel_size=[
                                3, 3], padding='valid')
...
def cyclegan_generator_resnet(images,
    ...
    with tf.contrib.framework.arg_scope(arg_scope_fn()):
        ...
        with tf.variable_scope('input'):
            ...
            net = layers.conv2d(net, num_filters, kernel_size=[
                                7, 7], padding='VALID')
        ...
        with tf.variable_scope('decoder'):
            ...
            with tf.contrib.framework.arg_scope(
                ...
                with tf.variable_scope('decoder1'):
                    net = upsample_fn(
                        net, num_outputs=num_filters * 2, stride=[2, 2])
                end_points['decoder1'] = net

                with tf.variable_scope('decoder2'):
                    net = upsample_fn(
                        net, num_outputs=num_filters, stride=[2, 2])
